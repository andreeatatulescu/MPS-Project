...
public class ThreadUtils {
    ...
    /**
     * This is a parent function for runThread to start the calculations on threads and
    ...
    public static void runner(List<File> filesList, int numberOfThreads, Thread[] threads,
        ...
        for (int t = 0; t < numberOfThreads; t++) {
            ...
            threads[t] = new Thread(() -> {
                try {
                    ThreadUtils.runThread(filesList, numberOfThreads, thread, operations, tree, result);
    ...
    /**
     * Function to obtain the scores of the files based on the thread.
    ...
    public static void runThread(List<File> filesList, int numberOfThreads, int thread,
                                  List<String> operations, Tree tree, List<Float> result) throws Exception {
        // Obtaining the start and the end for each thread to divide the files between each other.
        int start = (int) (thread * (double) filesList.size() / numberOfThreads);
        int end = (int) Math.min((thread + 1) * (double) filesList.size() / numberOfThreads, filesList.size());
        List<File> inFiles = new ArrayList<>(filesList.subList(start, end));
